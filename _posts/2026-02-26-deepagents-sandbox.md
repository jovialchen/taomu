...
1995 年，Java 随 JDK 1.0 发布了其安全模型，首次在应用层面引入了"sandbox"这个术语。Java 的安全架构将代码分为两类：本地代码（受信任，可以完全访问系统资源）和远程下载的 Applet（不受信任，只能在 sandbox 内运行）。
...
`BaseSandbox` 是一个抽象基类，采用了一种巧妙的设计模式：**将所有文件操作委托给 `execute()` 方法**。这意味着具体的沙箱实现只需要提供一个 `execute()` 方法，就能自动获得完整的文件操作能力。
